...
class FeatureState:
    ...
    async def handle_state_events(self, event: InputEvent, client_websocket: WebSocket, dispatcher: Dispatcher):
        ...
        async def handle_get():
            if event_data:
                if 'key' in event_data:
                    ...
                    await client_websocket.send_json(OutputEvent(
                        id = 'UPDATE_STATE',
        ...
        if event_id == 'GET_STATE': await handle_get()
        if event_id == 'SET_STATE': await handle_set()
        if event_id == 'SAVE_STATE': self.save_state()