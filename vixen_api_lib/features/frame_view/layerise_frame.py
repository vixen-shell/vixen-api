...
def set_level(level_key: LevelKeys | None):
    try: return getattr(Levels, level_key)
    except: return None

def set_anchor_edges(
        anchor_edge_key: AnchorEdgeKeys | None,
        alignment_key: AlignmentKeys | None
):
    if not anchor_edge_key: anchor_edge_key = 'full'
    if not alignment_key: alignment_key = 'center'
...
def layerise_frame(frame: Gtk.Window, namespace: str, layer_frame_params: LayerFrameParams):
    frame.set_app_paintable(True)
    init_style_context()

    frame.set_size_request(
        layer_frame_params.width or -1,
        layer_frame_params.height or -1
    )
    ...
    layerise_window(
        ...
        anchor_edges = set_anchor_edges(layer_frame_params.anchor_edge, layer_frame_params.alignment),
